...
public class ApplicationContextListener implements ServletContextListener {
    ...
    public void contextDestroyed(ServletContextEvent servletContextEvent) {
        ...
        stoppables.clear();
        Log.log("all threads stopped");
        Log.log("checking for changed data");
        boolean result = Application.checkIdChanged();
        result |= Application.getConfiguration().checkChanged();
        result |= Application.getContent().checkChanged();
        result |= Application.getUsers().checkChanged();
        if (!result){
            Log.log("nothing to save");
        }
        try {
            Thread.sleep(1000);
        } catch (Exception ignore) {
        }
        Log.log("ready after destroyed context");
    }
}
